...
export async function hostStartGame(settings) {
  const playersRef = ref(db, `rooms/${roomId}/players`);
  const snap = await get(playersRef);
  const players = snap.val() || {};
  const order = Object.keys(players);
  await set(ref(db, `rooms/${roomId}/meta`), {
    hostId: myUid, isPublic: roomMetaCache.isPublic || false, status: "lobby",
    ...
    turnOrder: order, turnIndex: -1, currentRoundNum: 1, createdAt: Date.now()
  });
  const metaRef = ref(db, `rooms/${roomId}/meta`);
  advanceTurn(metaRef, playersRef);
}
...
export function stopHostLoop() { clearInterval(hostTickInterval); }
